...
if not ANTHROPIC_API_KEY:
    logging.warning("ANTHROPIC_API_KEY is not set — Claude API calls will fail at runtime.")
...
# Invoke Anthropic Claude model (replaces AWS Bedrock)
def invoke_bedrock(prompt_text):
    """
    Drop-in replacement for the old invoke_bedrock() function.
    Uses the Anthropic Claude API instead of AWS Bedrock.
    The function name is kept the same so no other code needs to change.
    """
    try:
