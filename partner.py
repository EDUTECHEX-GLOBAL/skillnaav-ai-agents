...
async def shortlist_candidates(
    ...
    claude_tasks = []
    for r in results:
        if r and CLAUDE_LOWER <= r["similarity_score"] < CLAUDE_UPPER:
            claude_tasks.append(
    ...
    for r, verdict in zip(results, claude_verdicts):
        ...
        if CLAUDE_LOWER <= score < CLAUDE_UPPER and isinstance(verdict, dict) and verdict.get("claude_evaluated") is not None:
            r["ai_reasoning"] = verdict
            should_shortlist = verdict.get("shortlist", score >= ats_threshold)
        else:
            # Math is confident (score >= CLAUDE_UPPER or score < CLAUDE_LOWER)
            # Do NOT fabricate a Claude verdict — ai_reasoning = None tells the
            # frontend to show "—" instead of a misleading 🤖 badge.
            r["ai_reasoning"] = None
