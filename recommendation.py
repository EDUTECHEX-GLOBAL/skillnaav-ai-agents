...
2. For each internship write a short match_reason (1-2 sentences MAX) that tells
   the student exactly why it suits them — be specific, mention their actual skills
   or goals, not generic praise.
...
- match_reason must be personal and specific — never say "great opportunity" or "good fit".
- Return ONLY valid JSON. No explanation, no markdown, no code fences.

RESPONSE FORMAT (strict JSON array, {limit} items):
[
  {{
    "index": <original index from the candidates list>,
    "match_reason": "<1-2 sentence personalised explanation>"
  }},
  ...
]"""
